...
def topMatches(prefs, person, n=5, f = sim_pearson):
    scores = [(f(prefs,person,other), other)
              for other in prefs if other != person]
    scores.sort()
    scores.reverse()
    return scores[0:n]

def getRecommendations(prefs,person,f=sim_pearson):
    totals = {}
    simSums = {}
...
def calculateSimilarItems(prefs, n=10):
    ...
    for item in itemPrefs:
        ...
        scores = topMatches(itemPrefs, item, n = n, f = sim_distance)
        result[item] = scores
    return result
